...
def test():
    ...
    # We will use xregrid's own logic but add bounds
    ds_with_bounds = ds.cf.add_bounds(['latitude', 'longitude'])

    # Define target grid with bounds too
    ...
    regridder = Regridder(ds_with_bounds, target_grid_ds, method="bilinear")
    res = regridder(ds.air)

    zeros = np.sum(res.values == 0)
    print(f"Zeros with corners and SPH_DEG: {zeros}")
