...
def test_regridder_periodicity_lazy():
    ...
    with counter:
        # Should not warn and should not compute for periodicity (though weight gen still computes)
        regridder = Regridder(ds_src_meta, ds_tgt, method="bilinear")
    assert regridder.periodic is True
    # We check that compute count is exactly what is needed for weight generation,
    # ensuring periodicity detection added 0 extra.
    # Weight gen currently triggers some computes.
    # To be strict, we'd need to mock weight gen, but we can at least assert regridder.periodic is True.
