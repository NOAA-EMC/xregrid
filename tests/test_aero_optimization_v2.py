...
def test_optimization_v2_identity():
    ...
    da_clean = xr.DataArray(data_clean, dims=('lat', 'lon'),
    ...
    da_dirty = xr.DataArray(data_dirty, dims=('lat', 'lon'),
                          coords={'lat': src_grid.lat, 'lon': src_grid.lon})
...
def test_lazy_data_handling():
    ...
    da_lazy = xr.DataArray(data, dims=('lat', 'lon'),
                         coords={'lat': src_grid.lat, 'lon': src_grid.lon}).chunk({'lat': 9})
