...
def test_mixed_backend_dataset_regrid():
    ...
    with LocalCluster(n_workers=2, threads_per_worker=1) as cluster:
        with Client(cluster) as client:
            ...
            assert hasattr(regridder._weights_matrix, "key"), "Weights should be Dask Futures"
            ...
            assert not hasattr(ds_out.var_eager.data, "dask"), "var_eager should remain NumPy-backed"
            assert hasattr(ds_out.var_lazy.data, "dask"), "var_lazy should remain Dask-backed"
